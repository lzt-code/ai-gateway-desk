...
section('测试 25: sync 自动部署失败不中断')
{
  ...
  try {
    ...
    check(body.autoDeployed === null, 'autoDeployed === null（后台部署中，失败未知）')
    check(body.autoDeployError == null, 'autoDeployError 为空（部署在后台进行）')
...
section('测试 30: name 补救误报不触发 KV 部署')
{
  ...
  try {
    ...
    check(body.autoDeployed === true, 'autoDeployed === true（无真实变更，无需后台部署）')
